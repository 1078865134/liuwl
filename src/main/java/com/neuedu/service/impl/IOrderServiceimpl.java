...
import com.neuedu.vi.CartOrderItemVO;
import com.neuedu.vi.OrderItemVO;
import com.neuedu.vi.OrderVO;
import com.neuedu.vi.ShippingVO;
...
public class IOrderServiceimpl implements IOrderService {
    ...
    private ServerResponse getCartOrderItem(Integer userId,List<Cart> cartList){
        ...
        for(Cart cart:cartList){
            ...
            if(product==null){
                return ServerResponse.serverResponseByERROR("id为"+cart.getProductId()+"不存在");
            }
            if(product.getStatus()!= Const.ProductStatusEnum.PRODUCT_ONLINE.getCode()){
                return ServerResponse.serverResponseByERROR("id为"+cart.getId()+"的商品库存不足");
            }
            if(product.getStock()<cart.getQuantity()){
                return ServerResponse.serverResponseByERROR("id为"+cart.getProductId());
            }
            ...
            orderItem.setTotalPrice(BigDecialUtils.mul(product.getPrice().doubleValue(),cart.getQuantity().doubleValue()));
    ...
    //计算订单总价格
    private BigDecimal getOrderPrice(List<OrderItem> orderItemsList){
        BigDecimal bigDecimal = new BigDecimal("0");
        for(OrderItem orderItem:orderItemsList){
            bigDecimal=BigDecialUtils.add(bigDecimal.doubleValue(),orderItem.getTotalPrice().doubleValue());
    ...
    private OrderVO assembleOrderVO(Order order, List<OrderItem> orderItemList,Integer shippingId){
        ...
        if(shipping!=null){
            ...
            ShippingVO shippingVO = assebleShippingVO(shipping);
    ...
    //地址表
    private ShippingVO assebleShippingVO(Shipping shipping){
    ...
    public ServerResponse cancel(Integer userId,Long orderNo) {
        ...
        Order order = orderMapper.findOrderByUserIdOrderNo(userId, orderNo);
        if(order==null){
    ...
    public ServerResponse list(Integer userId,Integer pagenum, Integer pagesize) {
        ...
        if(orderList==null||orderList.size()==0){
            return ServerResponse.serverResponseByERROR("为查到订单信息");
        }
    ...
    public ServerResponse pay(Integer userId, Long orderNo) {
        // 测试当面付2.0生成支付二维码
        if(orderNo==null){
            ServerResponse.serverResponseByERROR("订单号不存在");
        }
        Order order = orderMapper.findOrderByOrderNo(orderNo);
        if(order==null){
            ServerResponse.serverResponseByERROR("订单不存在");
        }
        try {
...
//    =================================支付相关============================================
    ...
    public ServerResponse pay(Order order) throws IOException {
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                .setNotifyUrl("http://39.98.88.93:8080/order/alipay_callback.do")//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                String filePath = String.format("/ftpfile/qr-%s.png",
    ...
    public ServerResponse query_order_pay_status(Long orderNo) {
        if(orderNo==null){
            return ServerResponse.serverResponseByERROR("订单号不存在");
        }
        Order order = orderMapper.findOrderByOrderNo(orderNo);
        if(order==null){
